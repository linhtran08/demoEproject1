...
$(document).ready(function () {
   $('.add-to-cart').click(function (e) { 
    $('.cart-hover').remove('buzz');
    $('.cart-hover').hide();
    $('.cart-hover').show(200);
    $('.cart-hover').addClass('buzz');
   });
...
// Add item
    ...
    function displayCart() {
      ...
      var viewCart="";
      var orderSumary="";
      var emptyCart ="<div class='container'>"
      +"<div class='row'>"
      ...
      for(var i in cartArray) {
        output += "<div class='cart-list'>"
          ...
          +"<img class='media-object-cart' src='"+cartArray[i].img+"' alt='image' />"
          +"</a>"
          +"</div>"
          +"<div class='media-body-cart'>"
          +"<h4 class='media-heading'><a href=''>" + cartArray[i].name + "</a></h4>"
          +"<div class='cart-price'>"
          +"<div class='price-products btn btn-info'>$" + cartArray[i].price + "</div>"
          +"<div class='input-group quantity-products'>"
          +"<span class='input-group-btn'><button class='minus-item btn btn-primary' data-name='" + cartArray[i].name + "'>-</button></span>"
          +"<input type='number' min='0' class='item-count form-control' data-name='" + cartArray[i].name + "' value='" + cartArray[i].count + "'>"
          +"<span class='input-group-btn'><button class='plus-item btn btn-primary' data-name='" + cartArray[i].name + "' /,2ơpp>+</button></span>"
          +"</div>"
          +"<div class='total-products btn btn-success'>$" + cartArray[i].total + "</div>"
          +"</div>"
          +"</div>"
          +"<div style='flex: 1;'>"
          +"<a href='#' class='delete-item remove' data-name='" + cartArray[i].name + "'><i class='tf-ion-close'></i></a>"
          +"</div>"
          +"</div>";
          viewCart += "<div class='cart-list'>"
          +"<div style='flex: 2;'>"
          +"<a class='pull-left' href='#'>"
          +"<img class='media-object-cart' src='"+cartArray[i].img+"' alt='image' />"
          +"</a>"
          ...
          +"</div>"
          +"<div class='total-products btn btn-success'>$" + cartArray[i].total + "</div>"
          +"<div class='btn btn-danger delete-item remove' data-name='" + cartArray[i].name + "'><i class='tf-ion-close'></i></div>"
          +"</div>"
          +"</div>"
          +"</div>";
          orderSumary +="<div class='media product-card '>"
          +"<a class='pull-left' href='product-single.html'>"
          +"<img class='media-object' src='"+cartArray[i].img+"' alt='Image' />"
          +"</a>"
          +"<div class='media-body'>"
          +"<h4 class='media-heading'><a href='product-single.html'>" + cartArray[i].name + "</a></h4>"
          +"<p class='price'>"+ cartArray[i].count +" x $" + cartArray[i].price + " = $"+cartArray[i].total +"</p>"
          +"<span class='remove delete-item' data-name='" + cartArray[i].name + "'>Remove</span>"
          +"</div></div>";
      }
      if(cartArray.length>0){
        ...
        $('.block-cart').hide();
        $('.view-cart').hide();
        $('.show-cart-list').html('<h4 class="text-center">Your cart is currently empty.</h4>');
        $('.checkOut-list').html('<h4 class="text-center">Your cart is currently empty.</h4>');
      }
